...
pat_part = r'.+?\|ruslan-m\|(.+?)\|'
pat_speech = r'(.+?)\s(\.|\!|\?)'
files_li = os.listdir(r'C:\Users\Николай\Desktop\ex`,f\1 курс\практика ошибки\переведённые') #папка с файлами .cha
if not os.path.isdir(r'C:\Users\Николай\Desktop\ex`,f\1 курс\практика ошибки\переведённые'):  #папка с файлами .cha
    os.mkdir(r'C:\Users\Николай\Desktop\ex`,f\1 курс\практика ошибки\переведённые')  #папка с файлами .cha
with open(r'C:\Users\Николай\Desktop\ex`,f\1 курс\практика ошибки\mistakes.txt', 'w', encoding='utf-8') as f_mis: #док записи
    for f in files_li:
                    ...
                    if line.startswith('*'):
                        ...
                        if re.search(pattern, speech_cleaned):
                            for one_word in re.findall(pattern, speech_cleaned):
                                ...
                                f_mis.write(f'{file_name}: {ch_age}; {speaker_id}; "{speech_cleaned}"; {mis_w} -> {corr_w}\n') #на CSV у меня сил пока не хватило
print(counter)
